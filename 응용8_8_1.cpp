...
{ 
	...
	int  i, reel_num[3][3];
	int money=10000;
	srand(time(NULL));//금액에대한 초기회 
	printf("  슬롯머신 게임\n\n");
	display_rule();//3x3격자모양의 출력 
	gotoxy(3, 5);
	printf("릴1 릴2 릴3");
	gotoxy(1,6);
	draw_check02(3, 3);//각릴에 대한 첫번쨰 index 생성 
	for(i=0;i<3;i++)
		reel_num[0][i]=rand()%5;
	reel_series(reel_num);
	//현재 남은 금액표시 배팅금액 입력 슬롯머신의 릴 을 회전 
	do
	{
...
void draw_check02(int c, int r)//사각형 격자모양출력 
{
    int i, j;
...
void reel_series(int r[][3])//릴에대한 회전 순서를 결정 
{
	int i, j;
	for(i=0;i<3;i++)
		for(j=0;j<3;j++)
...
void gotoxy(int x, int y)//마우스제어 
{
   COORD Pos = {x - 1, y - 1};
   SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), Pos);
}
int game_progress(int money)//게임진행 상활출력,배팅금액입력 
{
    int bet;
	gotoxy(43, 17);
...
void display_reel(char rc[][3], int r[][3], int index)//화면에 릴출력 
{
	int i, j;
	for(i=0;i<3;i++)
		for(j=index;j<3;j++)
...
void clear_text(void)//화면에 출력된 문장지우기 
{
	int i, j;
	for(i=17;i<20;i++)
	{
...
//변수 초기화 릴을 화면에 display_reel게임 진행상황 출력 
	for(i=0;i<3;i++)
	{
		start=clock();
		do
		...
		num[i]=reel_num[1][i];
	}//각릴을 회전시켜 출력하고 임의의 키를 눌렀을경우 릴1초간격 시간차이 두고 멈추게함 
	getch();
	thank=return_money(num, bet, &case_num);//현재 금액출력 
	if (thank==0)
		*money-=bet;
	else
		*money+=thank;
