...
int main(void)
{
	...
	char user_name[4][7];//게임에 참가할 인원수입력 
	srand(time(NULL));
//변수초기화  
	printf("청기/백기 게임\n\n");
...
void gotoxy(int x, int y)//커서위치제 
{
   COORD Pos = {x - 1, y - 1};
   SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), Pos);
}
void game_control(int order, char name[7],어 char sol[][10], double check[][10], int quest)//게임제어문 
{
   char *flag[]={"청기", "백기"};
   ...
   display_rule();
   //문제용어를 배열에 저장 게임규칙출력 함수 display_rule 
   for(i=0;i<quest;i++)
   {
  	system("cls");
	rnd[0]=rand()%2;
...
void display_result(int user, int order[], double check[][10], char name[][7], char sol[][10], int quest)//결과표시 (시간,맞은개수) 
{
   int i, j, result[4]={0};
   double min_time[4]={0};
   system("cls");//사용자의 이름출력 
   printf("       ");
   for(i=0;i<user;i++)
	printf("%8s  ", name[order[i]]);
   printf("\n");
   for(j=0;j<quest;j++)//문제당 정오표시와 반응시간 출력 
   {
	printf("    %d번 : ", j+1);
	for(i=0;i<user;i++)
	{   
...
void display_rule(void)//키의 조작 방법 설 
{
	gotoxy(1,4);
