...
void game_control(void)//볼록을 좌우로 이동 시킴 
{
	int x, count=0;//볼록을 바닥으로 이동화되 해당위치에 볼록이 있으면 쌓임 
	system("cls");
...
{
	int y;
	//block stack[x]+=1과 같이 누적하고 누적된 높이로 블록을 이동 
	for(y=2;y<box_height+2-block_stack[x];y+=1)
	{//15회반복하는 동안 block_stack에 저장된 값을 쌓인 블록수로 표시 
		gotoxy(x, y);
		printf("□");
		Sleep(20);
		gotoxy(x, y);
