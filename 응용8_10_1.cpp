...
void gotoxy(int x, int y) //커서의 위치제어 
{
   COORD Pos = {x - 1, y - 1};
...
{
	//변수rnd 의 값에 따라 줄의 시작 위치를 결정하여
    //오른쪽으로 또는 왼쪽으로 이동하는 것처럼 표시
...
{
	//시작 위치를 나타내는 r_s 가 20 보다 크다는 것은 줄이 오른쪽으로 치우진 것이므로 승자는 t_name [1] 이 되고
	//그 반대의 경우는 t_name [ 가 승자 .r_s가 20 인 경우는 무승부
...
void game_control(int *r_s, clock_t st, double *ps) //게임의 제어 
{    
//r_s는 줄의 시작 위치를 나타내는 x 좌표의 값을 의미 . 
//변수 st 에는 게임의 시작 시간이 저장되고 , 종료 시간인 end 와의 차이 값으로 경과 시간 ps 를 계산
//변수 rnd 가 짝수이면 줄을 오른쪽 방향으로 이동할 길이로 계산하고 ,홀수이면 줄을 왼쪽 방향으로 이동시키기 위해 음수로 계산
	int rnd, i;
	clock_t end;
