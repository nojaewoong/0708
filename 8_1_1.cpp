...
void draw_check01(int c, int r)
{
    ...
    unsigned char b[12]; 
    for(i=1;i<12;i++)
	b[i]=0xa0+i; // 0xa1부터 0xa11까지의 값을 순서대로 지 
    printf("%c%c",a, b[3]);// 바둑판 첫번쨰 줄 출력 코드 
    for(i=0;i<c-1;i++)
	 printf("%c%c", a, b[8]);
    printf("%c%c", a, b[4]);
    printf("\n");
    for(i=0;i<r-1;i++) //두번재줄부터 마지막줄전까지 출력 코 
    {
	printf("%c%c", a, b[7]);
	for(j=0;j<c-1;j++)//마지막줄 출력  
	    printf("%c%c", a, b[11]);
	printf("%c%c",a, b[9]);
	printf("\n");
    }
